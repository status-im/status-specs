...
A `PNR message` (Push Notification Registration) MUST be sent to the [partitioned topic](waku-usage.md#partitioned-topic)
for the public key of the node, encrypted with this key.

The message MUST be wrapped in a [`ApplicationMetadataMessage`](payloads.md#payload-wrapper) with type set to `PUSH_NOTIFICATION_REGISTRATION`.
...
The response MUST be sent on the [partitioned topic](waku-usage.md#partitioned-topic) of the sender
and MUST not be encrypted using the [secure transport](secure-transport.md) to facilitate the usage of ephemeral keys.
...
The message MUST be wrapped in a [`ApplicationMetadataMessage`](payloads.md#payload-wrapper) with type set to `PUSH_NOTIFICATION_REGISTRATION_RESPONSE`.

A client SHOULD listen for a response sent on the [partitioned topic](waku-usage.md#partitioned-topic)
...
Using the topic derivation algorithm described [here](waku-usage.md#public-chats)
...
The message MUST be wrapped in a [`ApplicationMetadataMessage`](payloads.md#payload-wrapper) with type set to `PUSH_NOTIFICATION_QUERY_INFO`.

If no filtering is done based on public keys,
the access token SHOULD be included in the advertisement.
Otherwise it SHOULD be left empty.

This SHOULD be advertised on the [contact code topic](waku-usage.md#contact-code-topic)
...
To discover a push notification service for a given user, their [contact code topic](waku-usage.md#contact-code-topic)
...
The message MUST be wrapped in a [`ApplicationMetadataMessage`](payloads.md#payload-wrapper) with type set to `PUSH_NOTIFICATION_QUERY`.
...
An ephemeral key SHOULD be used and SHOULD NOT be encrypted using the [secure transport](secure-transport.md).
...
A `PushNotificationQueryResponse` message MUST be wrapped in a [`ApplicationMetadataMessage`](payloads.md#payload-wrapper) with type set to `PUSH_NOTIFICATION_QUERY_RESPONSE`.
...
The response MUST be sent on the [partitioned topic](waku-usage.md#partitioned-topic) of the sender
and MUST not be encrypted using the [secure transport](secure-transport.md) to facilitate
...
A `PushNotificationRequest` message MUST be wrapped in a [`ApplicationMetadataMessage`](payloads.md#payload-wrapper) with type set to `PUSH_NOTIFICATION_REQUEST`.
...
A `PushNotificationResponse` message MUST be wrapped in a [`ApplicationMetadataMessage`](payloads.md#payload-wrapper) with type set to `PUSH_NOTIFICATION_RESPONSE`.
...
`installation_id`: the [`installation_id`](account.md) of the device
...
`message_id`: the [status message id](payloads.md)
...
A client MAY share their public key through [contact updates](payloads.md#contact-update)
...
This can be mitigated by [device syncing](payloads.md), but not completely
addressed.
